...
system_mat, system_result = linalg.line_triangulation_system(dirs_norm, stations)

# Convenice function that computes the system and also solves the linear system
closest_point = linalg.solve_line_triangulation(dirs_norm, stations)
...
for ind in range(stations.shape[1]):
    st = stations[:, ind]
    ax.plot([target[0], st[0]], [target[1], st[1]],
            [target[2], st[2]], "--k", alpha=0.3)
