...
class Interpolator(ABC):
    ...
    @abstractmethod
    def get_state(self, t: npt.NDArray) -> npt.NDArray:
        pass
...
def legendre8(
    table: npt.NDArray, t1: int | float, tN: int | float, t: npt.NDArray, ti: Optional[npt.NDArray] = None
) -> npt.NDArray:
    ...
    den = np.array([40320.0, -5040.0, 1440.0, -720.0, 576.0, -720.0, 1440.0, -5040.0, 40320.0])
    trel = (t - t1) / (tN - t1) * (M - 1)
    ...
    for val, ix0, count in zip(u, ix, counts):
        ...
        zz = np.where(num == 0)[0]
        if len(zz):
            rval[six][zz] = table[np.round(trel[six][zz] - val).astype(int) + val]
    np.seterr(**err_save)
...
def legendre8_loop(table: npt.NDArray, t1: int | float, tN: int | float, t: npt.NDArray) -> npt.NDArray:
    """Order-8 Legendre polynomial interpolation
